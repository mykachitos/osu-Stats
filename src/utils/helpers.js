...
export function computeSkills(player) {
  ...
  const aim = Math.round(Math.min(99,
    40 + ((topPlay?.stars || 0) / 10) * 40 + (recentAvgAcc - 90) * 1.5 + highStarPlays * 3
  ));
  const speed = Math.round(Math.min(99,
    35 + (hasHighBPM ? 15 : 0) + ppFactor * 0.4 + recentAvgStars * 5
  ));
  const accScore = Math.round(Math.min(99,
    (recentAvgAcc - 88) * 5.5 + 20
  ));
  const stamina = Math.round(Math.min(99,
    30 + (playCount / 500) + (recentActivity.filter(p => parseFloat(p.acc) > 96).length * 6)
  ));
  const reading = Math.round(Math.min(99,
    40 + recentAvgStars * 6 + (accuracy - 90) * 1.2
  ));
  const tech = Math.round(Math.min(99,
    35 + (recentActivity.filter(p =>
  ...
  return maps
    .map(m => {
      ...
      const matchesWeak = weakSkill
        ? m.tags.some(t =>
            t.toLowerCase().includes(weakSkill.skill.toLowerCase().slice(0, 3))
          )
        : false;
